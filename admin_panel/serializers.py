...
class UserSerializer(serializers.ModelSerializer):
    ...
    # 'assigned_role' is used for outgoing data (GET) - matches your React fetch logic
    ...
    def create(self, validated_data):
        ...
        # 2. Create the User (This triggers the signal to create the Profile)
        user = User.objects.create_user(**validated_data)
        user.set_password(password)
        user.save()
        
        # 3. Update the role in the profile (Signal already created the profile)
    ...
    def update(self, instance, validated_data):
        # 1. Extract role and password if they exist
        ...
        # 3. Handle password update (if provided in the React edit form)
        if password:
...
class OrderItemSerializer(serializers.ModelSerializer):
    # This pulls the food name (e.g., "Burger")
    menu_item_name = serializers.CharField(source='menu_item.name', read_only=True)

    class Meta:
        model = OrderItem
        # Note: 'id' is the order item ID, 'menu_item' is the food ID
        fields = ['id', 'menu_item', 'menu_item_name', 'quantity', 'price']

class OrderSerializer(serializers.ModelSerializer):
    # This MUST be named 'items' to match the related_name='items' in your models.py
    items = OrderItemSerializer(many=True, read_only=True)
    
    # This creates the summary text like "3x Burger, 5x Momo" for your Admin Cards
