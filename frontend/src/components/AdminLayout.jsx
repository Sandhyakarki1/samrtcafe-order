...
const AdminLayout = () => {
  ...
  // --- 1. NEW STATE FOR AUTOMATIC USER DATA ---
  const [currentUser, setCurrentUser] = useState({
    username: 'Admin',
    email: 'sandhyakarki506@gmail.com' 
  });
  
  // Get initial data from localStorage
  const adminUser = JSON.parse(localStorage.getItem("admin_user")) || {};

  // --- 2. FETCH LATEST DATA AUTOMATICALLY ---
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        // This talks to  Django backend to get the latest info
        ...
        if (response.ok) {
          ...
          setCurrentUser({
            ...
            email: adminUser.email || 'sandhyakarki506@gmail.com'
          });
        }
      } catch (err) {
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-slate-100 h-24 flex items-center justify-between px-10 z-30 shadow-sm">
          <div>
            ...
            <p className="text-[11px] text-indigo-600 font-black uppercase tracking-[0.2em] mt-2 italic">
            ...
            <div className="relative">
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-3 w-64 bg-white rounded-[24px] shadow-2xl border border-slate-50 py-3 z-50 animate-in fade-in slide-in-from-top-2 text-left">
                  <div className="px-5 py-3 border-b border-slate-50 mb-2 text-left">
                    <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest text-left">Account Email</p>
                    {/* AUTOMATIC EMAIL IN DROPDOWN */}
                    <p className="text-xs font-bold text-slate-700 truncate text-left">{currentUser.email}</p>
                  </div>
                  
                  <button 
                    onClick={() => { setIsModalVisible(true); setIsProfileOpen(false); }}
                    className="flex items-center gap-3 px-5 py-2.5 w-full text-slate-600 hover:bg-indigo-50 text-sm font-bold transition-all"
                  >
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[40px] p-10 w-full max-w-sm shadow-2xl relative animate-in zoom-in duration-300">
            ...
            <div className="text-center">
              <div className="w-24 h-24 bg-indigo-600 text-white rounded-[30px] flex items-center justify-center text-4xl font-black mx-auto mb-6 shadow-xl">
                {currentUser.username.charAt(0)}
              </div>
              
              <h2 className="text-2xl font-black text-slate-800 tracking-tighter capitalize">
              ...
              <div className="bg-slate-50 rounded-3xl p-6 text-left space-y-4 mb-8">
                <div>
                  ...
                  {/* AUTOMATIC EMAIL IN MODAL */}
                  <p className="text-sm font-bold text-slate-700 truncate">{currentUser.email}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Account Security</p>
                  <p className="text-sm font-bold text-emerald-500 flex items-center gap-2">
                    <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div> Verified Profile
                  </p>
                </div>
              </div>
              ...
              <button 
                onClick={() => setIsModalVisible(false)}
                className="w-full py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-lg"
              >
                Close Profile
              </button>
            </div>
