...
import { CheckCircle, DollarSign, Printer, Receipt, Clock, Search } from 'lucide-react';
...
const Billing = () => {
  ...
  const fetchServedOrders = async () => {
    try {
      const res = await fetch(`${BASE_URL}/api/orders/`, { headers: NGROK_HEADER });
      const data = await res.json();
      // show orders that are 'Served' but not yet 'Paid'
  ...
  const handleSettlePayment = async (orderId) => {
    ...
    try {
      ...
      if (res.ok) {
        alert("Payment Successful! Table is now marked as free.");
        fetchServedOrders();
      }
    } catch (err) {
      alert("Connection error. Check MacBook/Ngrok.");
    }
  };
      ...
      <div className="flex justify-between items-end mb-10">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-3">
            ...
            <input 
                ...
                className="outline-none text-sm font-bold w-32"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {orders.filter(o => o.table_number.toString().includes(searchTerm)).length === 0 ? (
           <div className="col-span-full py-20 text-center bg-white rounded-[40px] border-2 border-dashed border-slate-200">
              ...
              <p className="text-slate-400 font-black uppercase tracking-widest text-xs">No pending bills</p>
           </div>
        ) : (
          orders.filter(o => o.table_number.toString().includes(searchTerm)).map((order) => {
            
            //  INCLUSIVE VAT MATH (13%)
            const grandTotal = parseFloat(order.total_price);
            const netAmount = grandTotal / 1.13;
            const vatAmount = grandTotal - netAmount;

            return (
              <div key={order.id} className="bg-white rounded-[40px] p-8 shadow-xl border border-white relative overflow-hidden group hover:scale-[1.02] transition-all">
                ...
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] font-black text-emerald-500 uppercase flex items-center gap-1 justify-end">
                        <CheckCircle size={10}/> Food Served
                    </p>
                  </div>
                </div>
                ...
                {/* ITEMS LIST */}
                <div className="mb-8">
                   <p className="text-[10px] font-black text-slate-400 uppercase mb-2 tracking-tighter">Items consumed</p>
                   <p className="text-sm font-bold text-slate-700 leading-relaxed italic border-l-4 border-indigo-100 pl-3">
                      {order.items_text}
                   </p>
                </div>

                {/* THE RECEIPT (Dashed Box) */}
                <div className="bg-slate-50 rounded-3xl p-6 mb-8 font-mono border-2 border-dashed border-slate-200 text-xs">
                    ...
                    <div className="flex justify-between text-slate-900 font-black text-xl border-t border-slate-200 pt-4">
                        <span className="tracking-tighter uppercase">Total Bill:</span>
                        <span>Rs. {grandTotal.toFixed(0)}</span> 
                    </div>
                    <p className="text-[8px] text-center mt-4 opacity-40 uppercase font-bold tracking-widest italic">VAT Inclusive Price Model</p>
                </div>
                ...
                <div className="flex gap-2">
                    <button 
                        ...
                        className="flex-1 bg-[#00D161] text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-emerald-100 flex items-center justify-center gap-2 active:scale-95 transition-all"
                    >
                        <DollarSign size={16}/> Settle Cash
                    </button>
                    <button 
                        onClick={() => window.print()}
                        className="bg-slate-100 text-slate-400 p-4 rounded-2xl hover:bg-indigo-50 hover:text-indigo-600 transition-all"
                    >
                        <Printer size={18}/>
                    </button>
                </div>
